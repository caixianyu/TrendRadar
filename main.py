...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .expand-icon { color: #667eea; transition: transform 0.3s; font-size: 18px; }
...
<body>
    ...
    html_parts.append(f'生成时间: {now.strftime("%Y-%m-%d %H:%M:%S")} | 点击新闻卡片查看详情')
    html_parts.append('</p>')
    ...
    for platform_name in platform_news.keys():
        ...
        html_parts.append(f'<button class="filter-btn" onclick="filterPlatform(\'{safe_name}\')">  {html_escape(platform_name)}</button>')
    ...
    for platform_name, news_list in sorted(platform_news.items(), key=lambda x: len(x[1]), reverse=True):
        ...
        for idx, news_data in enumerate(sorted_news):
            ...
            card_id = f"{safe_platform}-{idx}".replace(" ", "-")
            
            html_parts.append(f'''
            <div class="news-card" onclick="toggleCard('{card_id}')" id="card-{card_id}">
    ...
    # 保存文件
    file_path = get_output_path("html", "all_news.html")
